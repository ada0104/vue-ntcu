...
<body>
    ...
    <div id="competency" class="content2-Box">
        <template v-if="main=='unit'">
            <p v-if='bshowSuggest'>建議橫放螢幕以獲得最佳使用體驗！</p>
            ...
            if (this.mission_sn) {
                ...
                axios.post("modules/assignMission/assignment/competencyScore.php", formatToForm({ mission_sn: this.mission_sn, main: this.main }))
                    .then(response => {
                        ...
                        //輪播不同張表格給予不同資料 深拷貝原始資料
                        this.scoreTable =JSON.parse(JSON.stringify(response.data[4]))
                        this.scoreTable1 =JSON.parse(JSON.stringify(response.data[4]))
                        this.scoreTable2 =JSON.parse(JSON.stringify(response.data[4]))
                        
                        //產生第二張輪播表格資料
                       for(i=0;i<this.scoreTable.length;i++){
                            this.temp.push(this.scoreTable[i]['avg'].slice(3,7));
                            this.scoreTable1[i]['avg'] = [];
                            this.scoreTable1[i]['avg']= this.temp[i];
                        }
                        //產生第三張輪播表格資料
                       for(i=0;i<this.scoreTable.length;i++){
                            this.temp1.push(this.scoreTable[i]['avg'].slice(7,19));
                            this.scoreTable2[i]['avg'] = [];
                            this.scoreTable2[i]['avg']= this.temp1[i];
            ...
            updateIndex(index){
              //偵測雷達圖換頁時，給予不同表格標題以及資料範圍
                 if(index==0 && this.genres == "cps"){
                    ...
                    this.scoreTable = this.scoreTable1
                    ...
                    this.scoreTable = this.scoreTable2
                }
            },
            open() {
                //雷達圖說明彈出按鈕 
