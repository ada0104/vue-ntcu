...
// 		case "TEACHER": 
//       if($tea_num==0){
//          $whereDsc = " WHERE `teacher_user`='".$_SESSION['swTeacherNum']."' AND `student_user`>0  AND `timelist_num` IN ('0','46','62') ";
//       }else{$whereDsc = " WHERE `teacher_user`='".$_SESSION['swTeacherNum']."' AND `student_user`>0  AND `timelist_num` IN (".$f_num_dsc.") ";}
...
// 	}	
  ...
  $tea_num=$_SESSION['swTeacherNum']; 
	switch($_SESSION['loginType']){
      ...
      if($tea_num==0){
         $whereDsc = " WHERE `teacher_user`='".$_SESSION['swTeacherNum']."' AND `student_user`>0  AND `timelist_num` IN ('0','46','62') ";
      }else{$whereDsc = " WHERE `teacher_user`='".$_SESSION['swTeacherNum']."' AND `student_user`>0  AND `timelist_num` IN ($f_num_new)";}
    ...
    <div class="accordionButton">
        <table class="list_item">
            <tr>
                ...
                <td width="20%">                                   
                    <a class="button" title="觀看成績" href="record_s.php?t=<?php echo $getKey;?>&s=<?php echo $valueArray['num'];?>">觀看成績</a>                
                </td>
            </tr>
        </table>             
    </div>
        ...
        <table class="list_item">
            <tr>
				...
				if(isset($test_count[$valueArray['num']])){
					...
					if($total_test_num>0){
						echo $test_count[$valueArray['num']]."/".($total_test_num);
					}else{
						echo $test_count[$valueArray['num']]."/".($total_test_num);;
					}
				}else{
                ...
                <td width="20%">
                    <a class="button" title="學生表現" href="record_s.php?t=<?php echo $getKey;?>&s=<?php echo $valueArray['num'];?>">學生表現</a>                
                </td>
            </tr>
        </table>             
    </div>
